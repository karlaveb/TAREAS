...
# ╔═╡ f84db1aa-88f7-441e-b967-9542a71b5a46
@bind l5 Slider(0.05:0.05:2, default = 0.5)

# ╔═╡ e9ad65df-db42-4de1-b7de-0304f378024a
begin
	plot(sin, -π:l5:π, title = "Funciones trigonométricas", color = "violet", label = "sin")
	plot!(cos, -π:l5:π, color = "light blue", label = "cos")
end
...
quiver( (-2:0.2:2)', (-2:0.2:2) , quiver=g, color=:violet)

# ╔═╡ fe8a4bb1-12d4-4ab0-8cef-255404a40671
md"""

Donde la apostrofe fue utilizada para transponer la matriz (-2:0.2:2), pues un vector renglón es equivalente a una matriz de un renglón.
...
# ╔═╡ 991fc08a-e30d-4c87-b1b6-a1f7d7760b96
contour(0:0.05:2π,0:0.05:2π,h, fill=true, color = :gist_rainbow)
...
# Grafica los puntos [0,0,0], [1,1,1], ..., [10,10,10]
...
# ╔═╡ 85dd9bcb-d5bf-441f-960f-3d0d0f2c70ab
...
T = (r/G) * (sin(θ) + sqrt((sin(θ))^2 + 2 * ((G * H)/(r^2)))) 
x_m = (r^2/G) * (sin(θ) + sqrt((sin(θ))^2 + 2 * ((G * H)/(r^2)))) * cos(θ) 
y_m = ((r^2) * sin(θ)^2)/(2 * G) + H 


function o(t) 
        return (H + r_y * (t/r_x) - (1/2) * G * (t/r_x)^2)
end

q = H
function p(t)
        return q =+ (H + r_y * (t/r_x) - (1/2) * G * (t/r_x)^2)
end

@gif for t in (0:0.025:T)
	plot(o, r_x * (0:0.01:t), xaxis = (0, x_m), yaxis = (0, y_m), style = (:dot), color = "purple", lj = (1), legend = false, title = ("Tiro Parabólico"), xlabel = L"x", ylabel = L"y")
	scatter!(p, r_x * t, r_x * t, xaxis = (0, x_m), yaxis = (0, y_m), color ="purple", ms = (10), legend = false)
    end
end
...
# ╔═╡ 9e70d0ce-7798-47ad-b20d-21106b50df75
md"Se repite el proceso de la celda anterior. ?)"
...
@gif for n in (0:0.1:4)
	surface((0:0.1:2π), (0:0.1:3 * π * n), f, yaxis = (0, y_max), zaxis =(-1,1), color = :oslo)
    end
end
